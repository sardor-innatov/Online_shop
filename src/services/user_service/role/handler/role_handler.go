...
import (
	...
	"online_shop/src/services/user_service/role/service"
...
func NewRoleHandler(conn *pgxpool.Pool, group route.Group) {

	handler := &roleHandler{
		service: service.NewRoleService(conn),
	}

	scoreGroup := group.Group("/role", middleware.RequireAuth, middleware.CheckPermission(conn))
	{
		scoreGroup.Handle("POST", "", handler.Create)
...
// @Router       /role [POST]
func (h *roleHandler) Create(ctx route.Context) error {

	id, err := h.service.CreateRole(ctx)
	{
		if err != nil {
			return err
		}
